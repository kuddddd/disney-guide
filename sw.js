...
// Recebe mensagem da página principal e exibe como notificação nativa
self.addEventListener('message', function(e) {
  if(!e.data || e.data.type !== 'WDW_NOTIFY') return;
  var title = e.data.title || 'WDW Guide';
  var opts  = {
    body:    e.data.body  || '',
    icon:    e.data.icon  || '/icon-192.png',
    badge:   '/icon-192.png',
    tag:     e.data.tag   || 'wdw-alert',     // tag igual substitui notif anterior do mesmo tipo
    renotify: true,
    vibrate: [200, 100, 200],
    data:    e.data.data  || {},
  };
